<?php 
    ...
    $obtencion2 = "SELECT * FROM publicaciones_fr WHERE id_publicacion != '$id' and categoria = 'opt2' LIMIT 5";
    $obtencion_comen = "SELECT * FROM comentarios_fr WHERE id_pub = '$id'";
...
<body id="page-top">
    <header class="gastronomia">
        ...
        <section class="textos-header">
            ...
            <h2>En savoir plus sur les lieux que vous pouvez visiter</h2>
        </section>
                ...
                <div class="col-md-8" style="text-align: justify;">
                    <div class="rounded-3 contenedorPost">
                        <?php foreach($publicaciones as $publicacion): ?>
                            ...
                            <h4>Auteur: <?= $publicacion['autor_blog'] ?></h4>
                            ...
                            <div style="clear: both;overflow: hidden;">
                                ...
                                <h3>
                                    Évaluation: <img style="width: 8pc;" src="../src/img/estrellas_<?=$publicacion['rating']?>.png">
                                </h3>                               
                            </div>
                        <?php endforeach ?>
                    </div>
                    <div class="mt-3 rounded-3 contenedorComentarios">
                        ...
                        <?php if(isset($_SESSION['nameR_user'])): ?>
                            <form class="mt-3" action="procesarComentario.php?id=<?php echo $id_pub; ?>" method="POST">
                                <textarea class="form-control" style="height: 100px;" name="comentario" placeholder="Commentaire..."></textarea>
                                <input class="btn btn-secondary mt-2" type="submit" value="Commentaire">
                            </form>
                        <?php else: ?>
                            <a href="login.php" style="cursor: pointer;"><textarea class="form-control mt-3" disabled style="height: 100px; cursor: pointer;" name="comentario" placeholder="Connectez-vous pour commenter"></textarea></a>
                        <?php endif ?>
                    </div>
                </div>
                <div class="col-md-4">
                    <div>
                        <center>
                            <h2 class="text-center">Plus de Gastronomie</h2>
                            <?php foreach($publicaciones2 as $publicacion2): ?>
                                <div class="card mt-3 p-1 text-dark bg-opacity-25" style="width: 18rem; text-align: left; background: #EBE5DC;">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <p class="card-text">Auteur: <?= $publicacion2['autor_blog'] ?></p>
                                        <p class="card-text"><?= $publicacion2['resumen_blog'] ?>...</p>
                                        <p class="card-text"><small><?= $publicacion2['date_created'] ?></small></p>
                                        <center><a class="btn btn-outline-dark" href="gastronomiaPost.php?id=<?php echo $publicacion2['id_publicacion'];?>" role="button"><i class="bi bi-info-circle"></i> Voir plus...</a></center>
                                    </div>
                                </div>
                            <?php endforeach ?>
                        </center>
